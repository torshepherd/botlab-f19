...
robot_path_t MotionPlanner::directlyPlanPath(const pose_xyt_t& start, 
                                     const pose_xyt_t& goal, bool flagPushGoal)
{
    return search_for_path(start, goal, distances_, searchParams_, flagPushGoal);
}

bool MotionPlanner::checkPath(const robot_path_t& path) const {
    bool flagValidPath = true;

    bool flagIncludeUninitialized = true;
    for (size_t i = 0; i < path.path.size(); i++) {
        int currX, currY, currId;
        distances_.toInt(path.path[i].x, path.path[i].y, currX, currY);
        currId = distances_.cellIndex(currX, currY);
        if (distances_.isObstacle(currId, searchParams_.minDistanceToObstacle, flagIncludeUninitialized)) {
            flagValidPath = false;
            break;
        }
    }
    return flagValidPath;
}
                                     ...
                                     const SearchParams& searchParams)
{
    ...
    return search_for_path(start, goal, distances_, searchParams, flagPushGoal);
}


robot_path_t MotionPlanner::planPath(const pose_xyt_t& start, const pose_xyt_t& goal)
{
