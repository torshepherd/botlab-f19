...
#include <lcmtypes/block_request_t.hpp>
#include <lcmtypes/mbot_picking_status_t.hpp>
...
private:
    ...
    int currFrontier_; // Current frontier in the map
...
//    int8_t path_redundancy_count;
    int64_t planningTime_;

    bool flagGrab_;
    bool haveMap_;                   // Flag indicating if a map has been received since the last call to copyDataForUpdate
    bool flagGrabFinish_;
    int grabBlockNum_;
    int maxGrabBlockNum_;
    int16_t blockId_;
    bool haveTargetBlockOrPose_;
    bool flagNewBlockRequestMessage_;
    bool flagGrabStatusUpdateMessage_;
    bool flagPoseStackInitialized_;
    int8_t nextStateFromArm_;
    pose_xyt_t blockPose_;
    pose_xyt_t putPose_;
    std::vector<pose_xyt_t> poseStack_;
    pose_xyt_t grabTarPose_;

    block_request_t block_request_;
    block_request_t block_request_back_;
    mbot_picking_status_t grab_status_;
    ...
    void printPath(const robot_path_t& thePath);
    ...
    bool nextBlockOrPose(int16_t& blockId, pose_xyt_t& blockPose);
    void grabStateMachine(void);
    int8_t grabInitializing(void);
    int8_t grabReadyForNext(void);
    int8_t grabDriveToPose(void);
    int8_t grabPickingUp(void);
    int8_t grabReturnHome(bool stateChanged);
    int8_t grabPuttingDown(void);
    int8_t grabFailed(void);
    int8_t grabCompleted(void);
    bool publishGrabState(int8_t nextState, int publishTime = 2);
    bool subscribeGrabState(int8_t& nextState);
    bool waitForArmMessage(int8_t& nextState, int64_t waitTime);

    void handleBlockRequest(const lcm::ReceiveBuffer* rbuf, const std::string& channel, const block_request_t* block_request_back);
    void handleGrabStatusUpdate(const lcm::ReceiveBuffer* rbuf, const std::string& channel, const mbot_picking_status_t* grab_status);
