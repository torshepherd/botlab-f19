...
public:
    ...
    *   pose.
    */
    robot_path_t planPath(const pose_xyt_t& start, const pose_xyt_t& goal, const SearchParams& searchParams);

    robot_path_t directlyPlanPath(const pose_xyt_t& start, const pose_xyt_t& goal, bool flagPushGoal);
    ...
    *   pose.
    */
    robot_path_t planPath(const pose_xyt_t& start, const pose_xyt_t& goal);
    ...
    */
    ...
    inline pose_xyt_t poseFromIndex(int index) const {
        pose_xyt_t currPose;
        currPose.utime = 0;
        currPose.theta = 0.0;
        int currX, currY;

        distances_.cellXY(index, currX, currY);
        distances_.toFloat(currX, currY, currPose.x, currPose.y);
        return currPose;
    }

    inline pose_xyt_t poseFromIntXY(int currX, int currY) const {
        pose_xyt_t currPose;
        currPose.utime = 0;
        currPose.theta = 0.0;
    ...
    bool checkPath(const robot_path_t& path) const;
    ...
    */
    ...
    inline int newFrontier(const pose_xyt_t& start) const {
        return getFrontier(distances_, searchParams_, start);
    }

    inline bool isInitialized(int index) const {
        return ( !distances_.isUninitialized(index) );
    }

    inline void saveMap(const std::string& filename) const {
        distances_.saveToFile(filename);
    }

    inline bool isValidPose(const pose_xyt_t& start) const {
        int startX, startY;
        distances_.toInt(start.x, start.y, startX, startY);
        int startId = distances_.cellIndex(startX, startY);
        return (!distances_.isObstacle(startId, searchParams_.minDistanceToObstacle, true) );
    }

    inline robot_path_t pathToFree(const pose_xyt_t& start) const {
        return pathToFreeSpace(start, distances_, searchParams_);
    }
