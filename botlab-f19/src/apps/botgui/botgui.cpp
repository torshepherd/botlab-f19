...
, lcmInstance_(lcmInstance)
{
    ...
    f_odo = fopen("./odometry_data.csv", "w");
    f_slam = fopen("./slam_data.csv", "w");
    f_true_pose = fopen("./true_pose_data.csv", "w");
...
void BotGui::handlePose(const lcm::ReceiveBuffer* rbuf, const std::string& channel, const pose_xyt_t* pose)
{
    ...
    {
        if (startUTime_ < 0) {
            startUTime_ = pose->utime;
        }
        slamPose_ = *pose;
        float cur_time = (float) (utime_now()-startTime_);
        fprintf(f_slam, "%f, %f, %f, %f, %f\n", cur_time, static_cast<float>(pose->utime - startUTime_), pose->x, pose->y, pose->theta);
    }
...
void BotGui::handleOdometry(const lcm::ReceiveBuffer* rbuf, const std::string& channel, const odometry_t* odom)
{
    ...
    odometry_ = *odom;
    if (startUTime_ < 0) {
        startUTime_ = odom->utime;
    }

    float cur_time = (float) (utime_now()-startTime_)/1000;
    fprintf(f_odo, "%f, %f, %f, %f, %f\n", cur_time, static_cast<float>(odom->utime - startUTime_), odom->x, odom->y, odom->theta);
