...
enum PoseStrategy
{
    POSESTRATEGY_HIGHEST = 0,
    POSESTRATEGY_HIGHEST_N = 1
};
...
public:
    ...
    */
    ...
    float getDiffOdometry(const pose_xyt_t& currOdometry, const pose_xyt_t& prevOdometry, pose_xyt_t& diffOdometry) const;
    // void uniformDistributeParticles(const OccupancyGrid&   map);
...
private:
    ...
    PoseStrategy flagPoseStrategy_;         // Number of particles to use for estimating the pose
    float ratioParticles_;
    double bestWeight_;
    double bestWeightThres_;
    float odometryVelThres_;
    bool prevFlagNormalDistribution_;
    bool initialized_;
    pose_xyt_t prevOdometry_;
    bool flagGlobalLocalization_;
    int kNumParticlesOriginal_;
    std::default_random_engine generator_;

    std::vector<particle_t> resamplePosteriorDistribution(void);
    std::vector<particle_t> sameDistribution(void);
    std::vector<particle_t> computeProposalDistribution(const std::vector<particle_t>& prior);    
    bool computeNormalizedPosterior(const std::vector<particle_t>& proposal,
                                                                   const lidar_t& laser,
                                                                   const OccupancyGrid&   map,
                                                std::vector<particle_t>& posterior);

    pose_xyt_t estimatePosteriorPose(const std::vector<particle_t>& posterior, bool flagNormalDistribution);
    pose_xyt_t weightedAverage(const std::vector<particle_t>& posterior, float ratio) const;
    double nthWeight(const std::vector<particle_t>& posterior, float ratio) const;
    pose_xyt_t estimateSlamOdometry(const pose_xyt_t& odometry, pose_xyt_t& diffOdometry, const lidar_t& laser, const OccupancyGrid& map);
};
